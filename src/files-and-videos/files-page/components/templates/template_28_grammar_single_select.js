...
<body>
    ...
    <script>
        (function() {
            ...
            window.addEventListener('message', function(event) {
                ...
                if (event.data && event.data.type === 'quiz.get_answers') {
                    ...
                    if (window.parent) {
                        window.parent.postMessage({
                            type: 'pong',
                            data: { message: 'Template 28 is ready!', timestamp: new Date().toISOString() }
                        }, '*');
                    }
                }
            });
            ...
            // Send timer.start message after template loads (template 28 - no audio, start immediately)
            function sendTimerStart() {
                try {
                    if (window.parent) {
                        ...
                        console.log('✅ Sent timer.start message to parent (template 28 - after load)');
                    }
                } catch (error) {
