                    ...
                    if (audioElement.duration && timeSegments.length > 0) {
                        ...
                        if (currentTime >= currentSegment.end && !isTransitioning) {
                            ...
                            if (currentSegmentIndex < timeSegments.length) {
                                ...
                                // Send timer.start message to parent after audio completed (template 39)
                                try {
                                    if (window.parent) {
                                        window.parent.postMessage({
                                            type: 'timer.start',
...
export const getListenSingleChoiceTemplate = (questionText, optionsString, audioFile, timeSegments = '0-0', instructions = '音声を聞いて、正しい答えを選んでください。', scriptText = '', imageFile = '', countdownSeconds = 10, titleText = '') => {
